...
class PersonnagesManager
{
  ...
  public function get($info)
  {
    if (is_int($info))
    {
      $q = $this->db->query('SELECT id, nom, degats, exp, level, strength, type FROM personnages WHERE id = '.$info);
    ...
    {
      $q = $this->db->prepare('SELECT id, nom, degats, exp, level, strength, type FROM personnages WHERE nom = :nom');
      $q->execute([':nom' => $info]);
  ...
  public function getList($nom)
  {
    ...
    $q = $this->db->prepare('SELECT id, nom, degats, exp, level, strength, type FROM personnages WHERE nom <> :nom ORDER BY nom');
    $q->execute([':nom' => $nom]);
  ...
  public function update(Personnage $perso, $strength = 0)
  {
    ...
    $q = $this->db->prepare('UPDATE personnages SET exp = :exp, level = :level, strength = :strength, degats = :degats, type = :type WHERE id = :id');
    
    $q->bindValue(':exp', $perso->exp(), PDO::PARAM_INT);
    ...
    $q->bindvalue(':type', $perso->type(), PDO::PARAM_INIT);
